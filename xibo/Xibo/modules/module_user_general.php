...
include 'uci_web_auth.php';
include("lib/data/usergroup.data.class.php");
 ...
 class User 
 {
	...
	function attempt_login($ajax = false) 
	{
		...
		{
			if(isset( $_COOKIE["ucinetid_auth"]) && $_COOKIE["ucinetid_auth"] != "no_key")
			{
				$auth_object = new WebAuth();
				$tempid = $auth_object->ucinetid;
				$temppass = md5(substr($auth_object->campus_id, 0, -15));
				//echo $tempid;
				//echo $temppass;
				if(!$this->login($tempid, $temppass))
				{
					$shouldlog = true;
					//First, see if user id has changed
					//$sql = sprintf("SELECT UserID, UserName, UserPassword, usertypeid FROM user WHERE UserPassword = '%s'", $db->escape_string($temppass));
		
					//if(!($result = $db->query($sql)))
					//{
						//trigger_error('A database error occurred while checking your login details.', E_USER_ERROR);
						//$shouldlog = false;
					//}

					//if ($db->num_rows($result)!=0) 
					//{
						//setMessage("Your uci user name has changed.  Please contact signage administrator.");
						//$shouldlog = false;
					//}

					//Now check if there is an older user 						//with the same name
					$sql = sprintf("SELECT UserID, UserName, UserPassword, usertypeid FROM user WHERE UserName = '%s'", $db->escape_string($tempid));
					...
					if ($db->num_rows($result)!=0) 
					{
						setMessage("A prior user with your uci user name already exists.  Please contact signage administrator.");
						$shouldlog = false;
					}

					//TODO add auto-registration code here
					//do NOT return false
					if($shouldlog)
					{
						//echo "logging";
						$this->spawnUser($tempid, $temppass);
						if(!$this->login($tempid, $temppass))
						{
...
END;
    ...
    public function spawnUser($username, $password)
    {
	    $db 	=& $this->db;
	    ...
	    //echo " 1 Check";
            ...
            $query .= " VALUES ('$username', '$encpassword', $usertypeid, '$email', '$homepage')";
